    ...
    return (
      <View>
        <TouchableOpacity onPress={this.props.onBack}>
          <Text style={styles.backLink}>{I18n.t('back')}</Text>
        </TouchableOpacity>
        {deal && <Image source={{uri: deal.media[0]}} style={styles.image} />}
        <View>
          <View>{deal && <Text style={styles.title}>{deal.title}</Text>}</View>
          ...
          <View style={styles.description}>
            {deal && <Text>{deal.description}</Text>}
          </View>
        </View>
      </View>
    );
