...
class DealContainer extends Component<AppProps> {
  componentDidMount() {
    this.props.searchDeals();
  }

  currentDeal = (): UDeal => {
    return this.props.appData.deals.find(
      x => x.key === this.props.appData.currentDealId,
    );
  };
  ...
  render() {
    ...
    return (
      <View style={styles.main}>
        <SearchBar
          searchDeals={this.props.searchDeals}
          searchTerm={this.props.appData.searchTerm}
        />
        <DealList
          deals={this.props.appData.deals}
          onItemPress={this.props.setCurrentDeal}
        />
      </View>
    );
  }
...
function mapDispatchToProps(dispatch: any) {
  return {
    searchDeals: (searchStr: string = '') =>
      dispatch(AppAction.getDeals(searchStr)),
